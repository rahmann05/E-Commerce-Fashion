...
export async function GET() {
  ...
  try {
    const [
      ...
      fetch(`${COMMERCE_API_URL}/analytics`)
        .then((res) => res.json())
        .catch(() => ({ data: {} }))
    ]);
    ...
    const revenueGrowth =
      revPrevMonth === 0 ? 100 : Math.round(((revThisMonth - revPrevMonth) / revPrevMonth) * 100);
    ...
    const productsRes = await fetch(`${COMMERCE_API_URL}/products`)
      .then((res) => res.json())
      .catch(() => ({ data: [] }));
    const productMap = new Map(
      (productsRes.data || []).map((p: { id: string; name: string }) => [p.id, p.name])
    );
    ...
    const dailyMap = new Map(
      dailyRevenueRaw.map((d) => [d.createdAt.toISOString().split('T')[0], Number(d._sum.totalAmount)])
    );
