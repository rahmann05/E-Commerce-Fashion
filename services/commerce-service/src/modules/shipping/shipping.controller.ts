...
export class ShippingController {
  static calculateShipping(params: { lat: number, lng: number, city?: string }) {
    ...
    // Caps & Adjustments for realism
    if (distanceKm > 50) {
       feeJNE = 15000 + Math.floor((distanceKm - 50) * 150); 
       feeSiCepat = 18000 + Math.floor((distanceKm - 50) * 180);
    }
    
    if (distanceKm > 500) {
      feeJNE = Math.min(feeJNE, 85000); 
